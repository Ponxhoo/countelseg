...
//           console.log(signature.validTo_time_t +" fecha transformada"  +expirationDate);
  ...
  fetch("../get_signatures.php?opc=1")
    ...
    .then((data) => {
      ...
      data.forEach((signature) => {
        ...
        // Verifica si 'validTo_time_t' está definido
        if (signature.validTo_time_t) {
          console.log("Valor recibido:", signature.validTo_time_t); // Verifica qué valor exacto recibes

          // Si es un formato de fecha como 'YYYY-MM-DD', asegúrate de que sea reconocido como fecha
          // Añadimos 'T00:00:00' para asegurarnos de que se convierta en una fecha válida
          expirationDate = new Date(signature.validTo_time_t + 'T00:00:00');

          console.log("Fecha transformada:", expirationDate); // Para verificar la fecha transformada
        } else {
          expirationDate = null;
        }
        ...
        if (expirationDate) {
          ...
          console.log(
            signature.validTo_time_t + " fecha transformada " + expirationDate
          );
        ...
        var dateCell = document.createElement("td");
        dateCell.textContent = expirationDate
          ? expirationDate.toLocaleDateString() // Mostrar la fecha en formato local
          : "Fecha no disponible";
        signatureRow.appendChild(dateCell);
